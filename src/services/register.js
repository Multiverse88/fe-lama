...
export async function resendOtp(email) {
  try {
    ...
    const response = await api.post(`${API_BASE_URL}/resend-otp`, {
      email,
    });
...
export async function forgotPassword(email) {
  try {
    const response = await api.post(`${API_BASE_URL}/forgot-password`, {
      email,
    });
    return response.data;
  } catch (error) {
...
export const contact = async (formData) => {
  try {
    const response = await api.post(`${API_BASE_URL}/contact`, formData, {
      headers: {
        "Content-Type": "application/json",
      },
    });
